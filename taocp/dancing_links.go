...
func ExactCover(items []string, options [][]string, secondary []string,
	stats *ExactCoverStats) iter.Seq[[][]string] {
...
// of n values
func LangfordPairs(n int, stats *ExactCoverStats) iter.Seq[[]int] {
...
func NQueens(n int, stats *ExactCoverStats) iter.Seq[[]string] {
	...
	return func(yield func([]string) bool) {
		...
		// Generate solutions
		for solution := range ExactCover(items, options, sitems, stats) {
			x := make([]string, 2*n)
			i := 0
			for _, option := range solution {
				x[i] = option[0]   // row
...
func Sudoku(grid [9][9]int, stats *ExactCoverStats) iter.Seq[[9][9]int] {

	return func(yield func([9][9]int) bool) {
		var (
			i int // row number
			j int // column number
